...
当前在内存生成逻辑行并执行，未下推数据库索引。设快照行数 N、表达式节点数 E、匹配行数 M、排序键数 K，过滤约 O(N×E)，排序约 O(M log M×K)，另有行生成和投影成本。Qt 现在后台执行且支持取消，用表模型按需显示完整返回结果（Qt 行号上限 INT_MAX），不再使用 2000 行截断。Core 仍需存储全部逻辑行和结果，尚无磁盘分页排序。取消、过期结果保护及规模验证见 [P2-1](performance-and-interaction.md)。
