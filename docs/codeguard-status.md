...
已完成编译数据库自动发现、缺失引导和副本内 CMake 参数生成；C/C++ 编译器、目标、构建参数、规则启停、复制范围按工程保存。CLI/Qt 共用配置模型；Qt 提供工程设置、编译命令显式选择、INI 会话恢复，快照和构建历史关联当时配置。提交 `6ab64d1` 的 Windows CI `34710000310` 成功：Core 47 通过/1 跳过，Clang/Qt 74 通过/1 跳过，真实工程 44 个 CTest 及独立运行包检查全部通过，详见 [配置管理](project-configuration.md)。依赖搬运、构建阶段代码生成和所有配置联合建模不在已验收范围。

## P1-2 规则引擎

已完成五个独立规则模块、按工程启停和级别、带理由抑制、失效诊断、CLI/Qt 管理及 schema 5 持久化。45 个固定合成样例记录 TP 20 / FP 0 / TN 21 / FN 4，不能外推真实项目精度。提交 `664aa9f` 的 Windows CI `34712210281` 成功：Core 49 通过/1 跳过、Clang/Qt 84 通过/1 跳过；三个真实工程 44 个 CTest 全通过、59 个有效 TU 全解析，26 条提醒与复核位置一致。独立运行包及本机打包后规则 GUI 流程通过，完整范围见 [规则引擎](rule-engine.md)。下一项为 P1-3 报告与版本对比。
