...
已接入编译数据库自动发现、缺失引导和副本内 CMake 参数生成；C/C++ 编译器、目标、构建参数、规则启停、复制范围按工程保存。CLI/Qt 共用配置模型；Qt 提供四页设置、编译命令显式选择、INI 会话恢复，快照和构建历史关联当时配置。新增存储迁移、损坏保护、路径与宏变体选择、CLI/Qt 完整流程回归，本地已通过；最终 Windows CI 收尾见 [配置管理](project-configuration.md)。依赖搬运、构建阶段代码生成和所有配置联合建模不在已验收范围，后续优先级为 P1-2。
