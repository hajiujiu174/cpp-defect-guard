...
范围更新（2026-09-10）：用户确定当前只交付 Windows，跨平台新增工作暂缓。下文保留 2026-09-09 的历史验收记录；新增 Windows CI、锁定工具链及独立 runner 检查见 [Windows CI](windows-ci.md)。

后续进展（2026-09-11）：三个固定真实库工程的 P0-2 验收已完成，并增加 CLI CMake 定义及副本目录保留入口；当前结果、限制和新版回归计数见 [真实工程验收](real-project-acceptance.md)。下文的“尚未完成真实工程验证”描述仅代表 9 月 9 日历史状态。
