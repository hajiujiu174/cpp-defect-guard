...
std::vector<Item> items(const ScanResult& scan,const ScanContext& context){
    std::vector<Item> result;
    for(const auto& issue:scan.analysis.issues){context.check();result.push_back({&issue,false});}
    for(const auto& issue:scan.analysis.suppressed_issues){context.check();result.push_back({&issue,true});}
    return result;
}
const FileRecord* file(const ScanResult& scan,const std::string& path,const ScanContext& context){
    const auto found=std::find_if(scan.files.begin(),scan.files.end(),[&](const auto& f){context.check();return f.path==path;});
...
std::pair<std::string,std::string> absent(const ScanResult& before,const ScanResult& after,const Issue& issue,const ScanContext& context){
    const auto* current=file(after,issue.file,context);
    if(!current){
    ...
    const auto* previous=file(before,issue.file,context);
...
ScanComparison compare_scans(const ScanResult& before,const ScanResult& after,const ScanContext& context){
    ...
    const auto old=items(before,context),now=items(after,context);
    ...
    for(std::size_t i=0;i<old.size();++i){context.check();old_groups[key(*old[i].issue)].push_back(i);}
    for(std::size_t i=0;i<now.size();++i){context.check();new_groups[key(*now[i].issue)].push_back(i);}
    for(const auto& [group,left]:old_groups){
        ...
        std::map<std::pair<int,int>,std::vector<std::size_t>> sites;
        for(auto it=right->second.rbegin();it!=right->second.rend();++it){context.check();const auto& issue=*now[*it].issue;sites[{issue.line,issue.column}].push_back(*it);}
        for(const auto i:left){
            context.check();const auto& issue=*old[i].issue;auto site=sites.find({issue.line,issue.column});
            if(site!=sites.end()&&!site->second.empty()){const auto j=site->second.back();site->second.pop_back();matched[i]=static_cast<int>(j);used[j]=true;}
        }
    ...
    for(std::size_t i=0;i<old.size();++i){
        ...
        }else{auto [status,reason]=absent(before,after,*old[i].issue,context);change.status=std::move(status);change.reason=std::move(reason);}
        result.changes.push_back(std::move(change));
    }
    for(std::size_t i=0;i<now.size();++i){context.check();if(!used[i])result.changes.push_back({now[i].suppressed?"new_suppressed":"added","基线中未找到可可靠匹配的问题；本次新观测到",{},*now[i].issue});}
    return result;
}
} // namespace codeguard
