...
void run(const std::string& name,const std::string& cli){
    ...
    if(name=="config_migration"){
        ...
        f.sql("DROP TABLE unit_coverage; ALTER TABLE scan DROP COLUMN inventory_policy; ALTER TABLE analysis DROP COLUMN analyzer_revision; ALTER TABLE translation_unit DROP COLUMN command_fingerprint; DROP TABLE suppressed_issue; DROP TABLE rule_diagnostic; DROP TABLE project_configuration; ALTER TABLE analysis DROP COLUMN configuration; ALTER TABLE build_run DROP COLUMN compile_commands; ALTER TABLE build_run DROP COLUMN configuration; PRAGMA user_version=3;");
